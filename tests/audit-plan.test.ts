...
describe('design audit planner', () => {
  ...
  it('can isolate the user-facing content audit with localized, context-sensitive criteria', () => {
    ...
    expect(plan.checks[4]?.criterion).toMatch(/주장·결론·권고.*문구의 강도와 확신 수준.*근거/);
    expect(plan.checks[5]?.criterion).toMatch(/구체적이고 맥락에 맞는 정보.*무관한 제품이나 페이지/);
    expect(plan.checks[6]?.criterion).toMatch(/정보 가치·과업 관련성·근거 수준/);
  });
