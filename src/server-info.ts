...
export const SERVER_INSTRUCTIONS = [
  'Use Web Stylebook before implementing or redesigning UI. For an audit or redesign, when the human has not already set scope, first ask two short questions and stop: audit coverage (visual only / visual plus user-facing copy and information structure / full experience including behavior, accessibility, states, and build-performance evidence) and change depth (findings only / prioritized proposals / implement and verify). Commit, push, release, and deployment remain separate authorizations. Start with recommend_design_direction using product context, then read webstylebook://styles/{id}. Use search_design_references to find a small set of relevant real-world examples and get_design_reference for only the selected entries; treat their observations and tokens as attributed research evidence, not permission to copy or redistribute screenshots, brand assets, copy, typefaces, or visual identity. Then call get_design_principle_plan for the layout concerns/surface/phase, call get_ux_principle_plan for the task outcomes/surface/phase, call get_ui_state_plan for each surface, and compose_design_tokens for tokens. Prefer the pinned visual contract path: pass contract.schema from manifest.visualContract.contractSchema and optionally its contentHash plus the selected style revision; omit colorMode for the authored native mode and carry the website editor overrides. Keep the complete visualContract companion when delivering CSS or a theme. Legacy calls remain family-derived starting points. After implementation call get_design_audit_plan and inspect the actual rendered/code/interaction evidence for every included check; missing evidence is NOT_VERIFIED, never PASS. Submit the plan query, target matrix, evidence references, and one result per check/target to validate_design_audit_result before claiming completion; its valid field describes contract validity, not visual quality. When content is in scope, lack of internal jargon is not enough: test whether material claims, conclusions, and recommendations match their support and expressed certainty; whether prominent copy conveys concrete, context-specific information; and whether visual prominence matches information value, task relevance, and support. Do not invent evidence, facts, or next steps while rewriting. Use design principles as practical placement and visual-review guidance rather than a fixed recipe. Treat UX principles as evidence-labeled decision prompts, not universal laws; accessibility, safety, informed consent, and truthful feedback take priority.',
